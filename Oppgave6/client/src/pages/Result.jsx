...
const Result = () => {
  ...
  return (
    <section>
      <Heading mb={10} as="h1" fontSize="xl">Polling Results</Heading>
      {error && <p>{error}</p>}
      <Flex direction={"column"}>
        {polls &&polls.map((poll) => (
            <Box p="6" as="article" key={poll._id}>
              <Heading mb={2} as="h2" size="md" color="#007b5f">{poll.question}</Heading>
              ...
              <Text fontSize="sm" marginTop="10px" mb={2}>{new Date(poll.createdAt).toDateString()}</Text>
